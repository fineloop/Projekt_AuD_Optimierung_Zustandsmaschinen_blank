...
BOOL CDib::DibLoaded() const throw()
{
	return (m_pBmpInfoHeader!=NULL);
}
CSize CDib::GetDimensions() const throw()
{	
	if (!DibLoaded()) return CSize(0,0);
	return CSize((int)m_pBmpInfoHeader->biWidth,(int)m_pBmpInfoHeader->biHeight);
}
...
void CDib::Load(CFile* pFile) throw(CImageException)
{
	...
	if(m_pBmpFileHeader->bfType != 0x4d42) {
		Clear();
		pFile->Close();
		throw CImageException(string("Failed to load document"));
	} else {
		DWORD dwLength;
		...
		unsigned int dwDibLength = (unsigned int)pFile->GetLength() - m_pBmpFileHeader->bfOffBits / 4;
...
void CDib::Create(CBitmap* pBitmap) throw(CImageException)
{
	...
	if(rc == 0)
		throw CImageException(string("Can't get BITMAP from CBitmap*"));
	...
	// Color Table
	if((m_pBmpInfoHeader->biClrUsed == 0))
		m_uiNumColors = 1 << (m_pBmpInfoHeader->biBitCount-1);
	else
		m_uiNumColors = m_pBmpInfoHeader->biClrUsed;
	if(m_pBmpInfoHeader->biClrUsed == 0)
		m_pBmpInfoHeader->biClrUsed = m_uiNumColors;
...
void CDib::Save(CString fileName) throw(CImageException) 
{
	...
	if(file.Open(fileName, CFile::modeWrite | CFile::modeCreate) == 0)
		throw CImageException(string("Cannot open a file"));
	Save(&file);
}
...
void CDib::Draw(CDC* pdc, int x, int y, double scale, DWORD dwROPCode) const throw(CImageException) {
	...
	if(nScanLines == GDI_ERROR)
		throw CImageException(string("CDib::Draw exception"));
}
